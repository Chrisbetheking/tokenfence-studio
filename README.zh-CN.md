...
- **安全— Prompt 输入**：本地文件扫描和 Prompt 守卫，确保敏感信息不会泄。- **多模型路区**：支。OpenAI、Anthropic 和本地模。- **项目上下文打区**：将项目文件打包为上下文，供 LLM 使用
...
| 平台 | 文件 | 类型 |
|------|------|------|
| Windows | [TokenFence-Studio-Windows-v1.4.0-portable.zip](https://github.com/Chrisbetheking/tokenfence-studio/releases/latest/download/TokenFence-Studio-Windows-v1.4.0-portable.zip) | 便携。|

## 系统要求

- **操作系统**：Windows 10/11 (64-bit)
- **运行区**：WebView2 Runtime (通常已预装，如未安装会自动提。
...
- **已添加模区**：查看和管理已配置的模型，包括模型状态和 API Key 配置
- **模型区**：浏览可用模型，按提供商分类
- **提供区**：配。API 提供商（OpenAI、Anthropic 等）。API Key
- **路由**：管理模型路由规。
### 项目
