...
export function UpdatesScreen({ language }: { language: Language }) {
  ...
  return <main className="modern-page updates-page">
    ...
    <section className="update-hero"><div className={`update-orb ${info?.updateAvailable ? 'available' : ''}`}><Icon name={info?.updateAvailable ? 'download' : 'check'} size={30} /></div><span className="section-kicker">CHRIS STUDIO</span><h2>{info?.updateAvailable ? copy(language, `Version ${info.latestVersion} is available`, `发现新版本 ${info.latestVersion}`) : copy(language, 'You are on the latest published version', '当前已是最新发布版本')}</h2><p>{info?.errorMessage ?? copy(language, `Installed: ${info?.currentVersion ?? '2.0.0'} · Latest: ${info?.latestVersion ?? 'checking'}`, `已安装：${info?.currentVersion ?? '2.0.0'} · 最新：${info?.latestVersion ?? '检查中'}`)}</p>{info?.releaseUrl && <button className="button primary" onClick={() => void openExternal(info.releaseUrl!)}><Icon name="external" />{copy(language, 'Open GitHub Release', '打开 GitHub Release')}</button>}</section>
    ...
    <section className="update-settings-summary"><article><Icon name="git" /><div><strong>{copy(language, 'Repository-linked', '已连接仓库')}</strong><p>{copy(language, 'The desktop app reads the latest public GitHub Release and its downloadable assets.', '桌面应用会读取最新公开 GitHub Release 及其下载资源。')}</p></div></article><article><Icon name="shield" /><div><strong>{copy(language, 'No silent installation', '不会静默安装')}</strong><p>{copy(language, 'Chris Studio reports updates but never replaces the application without your action.', 'Chris Studio 只提示更新，不会在未经操作时替换应用。')}</p></div></article></section>
  </main>;
}
